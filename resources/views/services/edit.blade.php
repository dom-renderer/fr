...
@section('content')
    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="mt-4">
            <form method="POST" action="{{ route('services.update', $service->id) }}" id="editServiceForm">
                @method('patch')
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                    <input value="{{ old('name', $service->name) }}" 
                        type="text" 
                        class="form-control" 
                        name="name" 
                        id="name" 
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="pricing_type" id="pricing_type" class="form-control" required>
                            ...
                            <option value="fixed" {{ old('pricing_type', $service->pricing_type) == 'fixed' ? 'selected' : '' }}>Fixed</option>
                            <option value="as_per_actual" {{ old('pricing_type', $service->pricing_type) == 'as_per_actual' ? 'selected' : '' }}>As Per Actual</option>
                        </select>
                ...
                <div id="fixed_price_section">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input value="{{ old('price_per_piece', $service->price_per_piece) }}" 
                                type="number" 
                                step="0.01"
                                min="0"
                                class="form-control" 
                    ...
                    <div class="mb-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="price_includes_tax" value="1" id="price_includes_tax" {{ old('price_includes_tax', $service->price_includes_tax) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="tax_slab_id" id="tax_slab_id" class="form-control select2" required>
                            ...
                            @foreach($taxSlabs as $slab)
                                <option value="{{ $slab->id }}" {{ old('tax_slab_id', $service->tax_slab_id) == $slab->id ? 'selected' : '' }}>{{ $slab->name }}</option>
                            @endforeach
                        </select>
