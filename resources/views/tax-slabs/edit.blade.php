...
@section('content')
    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="mt-4">
            <form method="POST" action="{{ route('tax-slabs.update', $taxSlab->id) }}" id="editTaxSlabForm">
                @method('patch')
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                    <input value="{{ old('name', $taxSlab->name) }}" 
                        type="text" 
                        class="form-control" 
                        name="name" 
                        id="name" 
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-control">
                        <option value="1" {{ $taxSlab->status == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $taxSlab->status == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#cgst, #sgst').on('change keyup', function() {
             var validator = $("#editTaxSlabForm").validate();
