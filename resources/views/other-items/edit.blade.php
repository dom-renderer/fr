...
@section('content')
    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="mt-4">
            <form method="POST" action="{{ route('other-items.update', $otherItem->id) }}" id="editOtherItemForm">
                @method('patch')
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                    <input value="{{ old('name', $otherItem->name) }}" 
                        type="text" 
                        class="form-control" 
                        name="name" 
                        id="name" 
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="pricing_type" id="pricing_type" class="form-control" required>
                            ...
                            <option value="fixed" {{ old('pricing_type', $otherItem->pricing_type) == 'fixed' ? 'selected' : '' }}>Fixed</option>
                            <option value="as_per_actual" {{ old('pricing_type', $otherItem->pricing_type) == 'as_per_actual' ? 'selected' : '' }}>As Per Actual</option>
                        </select>
                ...
                <div id="fixed_price_section">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input value="{{ old('price_per_piece', $otherItem->price_per_piece) }}" 
                                type="number" 
                                step="0.01"
                                min="0"
                                class="form-control" 
                    ...
                    <div class="mb-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="price_includes_tax" value="1" id="price_includes_tax" {{ old('price_includes_tax', $otherItem->price_includes_tax) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="tax_slab_id" id="tax_slab_id" class="form-control select2" required>
                            ...
                            @foreach($taxSlabs as $slab)
                                <option value="{{ $slab->id }}" {{ old('tax_slab_id', $otherItem->tax_slab_id) == $slab->id ? 'selected' : '' }}>{{ $slab->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-control">
                        <option value="1" {{ $otherItem->status == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $otherItem->status == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
