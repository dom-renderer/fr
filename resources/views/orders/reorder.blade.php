                            ...
                            <div class="summary-row flex-column align-items-start d-none" id="tax_breakdown_section">
                                <div class="d-flex justify-content-between w-100 mb-1">
                                    ...
                                    <span class="text-danger small"
                                        id="cgstValueDisplay">+{{ Helper::defaultCurrencySymbol() }}0</span>
                                </div>
                                <div class="d-flex justify-content-between w-100">
                                    <span class="text-muted small">SGST (<span
                                            id="sgst_percentage_display">{{ $sgstPercentage ?? 0 }}</span>%):</span>
                                    <span class="text-danger small"
                                        id="sgstValueDisplay">+{{ Helper::defaultCurrencySymbol() }}0</span>
                                </div>
                    ...
                    <div class="card border-0 shadow-sm mb-3">
                        <div class="card-footer bg-white border-top">
                            <button type="submit" class="btn btn-primary w-100">
                                <i class="fas fa-check me-1"></i> Place Reorder
                            </button>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function calculateRowTotal($row) {
                ...
                $row.find('.row-total').text(CURRENCY_SYMBOL + (priceGiDi * qty).toFixed(2));
                updateSummary();
            }
            ...
            function updatePreview() {
                ...
                function addPreviewRow($row, typeLabel) {
                    ...
                    if (typeLabel !== 'Product') {
                        ...
                        displayPrice = CURRENCY_SYMBOL + basePrice.toFixed(2);
                        totalDisplay = baseTotal ? (CURRENCY_SYMBOL + baseTotal.toFixed(2)) : '-';
                        cgstDisplay = CURRENCY_SYMBOL + cgstAmt.toFixed(2);
                        sgstDisplay = CURRENCY_SYMBOL + sgstAmt.toFixed(2);
                    } else {
                        // Products: simple price display, no per-item tax
                        displayPrice = CURRENCY_SYMBOL + price.toFixed(2);
                        totalDisplay = total ? (CURRENCY_SYMBOL + total.toFixed(2)) : '-';
                        cgstDisplay = '';
                        sgstDisplay = '';
                    }
                    ...
                    if (hasItems) {
                        ...
                        $body.append(`
                            <tr class="preview-subtotal-row">
                                ...
                                <td class="text-end">${CURRENCY_SYMBOL + sectionTotal.toFixed(2)}</td>
                            </tr>
                        `);
                        ...
                        if (typeLabel === 'Product') {
                            ...
                            $body.append(`
                                <tr class="preview-cgst-row">
                                    ...
                                    <td class="text-end">${CURRENCY_SYMBOL + cgstVal.toFixed(2)}</td>
                                </tr>
                                <tr class="preview-sgst-row">
                                    <td class="ps-5">SGST (${sgstPercent}%)</td>
                                    <td class="text-end">${CURRENCY_SYMBOL + sgstVal.toFixed(2)}</td>
                                </tr>
                                <tr class="preview-grandtotal-row">
                                    <td class="pe-3">Products Total</td>
                                    <td class="text-end">${CURRENCY_SYMBOL + productsTotal.toFixed(2)}</td>
                                </tr>
                            `);
                        }
                        totalCount++;
            ...
            function updateSummary() {
                ...
                $('#subtotalDisplay').text(totalBasePrice.toFixed(2));
                $('#discountValueDisplay').text('-' + CURRENCY_SYMBOL + discountValue.toFixed(2));

                // Display Consolidated Tax
                $('#cgst_percentage_display').text(globalCgstPercent);
                $('#sgst_percentage_display').text(globalSgstPercent);
                $('#cgstValueDisplay').text('+' + CURRENCY_SYMBOL + totalCgst.toFixed(2));
                $('#sgstValueDisplay').text('+' + CURRENCY_SYMBOL + totalSgst.toFixed(2));

                $('#additionalChargesTotalDisplay').text(additionalChargesTotal.toFixed(2));
                $('#grandTotalDisplay').text(grandTotal.toFixed(2));

                $('#pendingAmountDisplay').text(CURRENCY_SYMBOL + balanceDue.toFixed(2));
                $('#balanceDueDisplay').text(CURRENCY_SYMBOL + balanceDue.toFixed(2));
