...
class StoresExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($store): array
    {
        return [
            ...
            'manager_first_name' => $store->dom->name ?? '',
            'manager_middle_name' => $store->dom->middle_name ?? '',
            'manager_last_name' => $store->dom->last_name ?? '',
            'ops_mgr' => '',
            'ops_head' => '',
            'model' => $store->modeltype ? $store->modeltype->name : '',
            'manager_id' => ($store->dom->employee_id ?? '') . '_' . ($store->dom->name ?? ''),
            'manager_mobile' => $store->dom->phone_number ?? '',
